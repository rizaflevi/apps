<?php
//	bm_rekap_ct.php
//	Laporan baca meter per petuga cater
...
//	include "preloader.php";
	...
	$cHEADER		= S_MSG('RQ81','Laporan Rekap Baca Meter per Petugas');
	...
	$c_FILTER_RAYON = '';
	$q_USER_RAYON=OpenTable('TbUser', "USER_CODE)='$_SESSION[gUSERCODE]' and DELETOR=''");
	...
	$cFILTER_AREA='';
	if ($c_RAYON=='') {
		$r_TB_AREA	= SYS_FETCH(SYS_QUERY("select * from tb_area where APP_CODE='$cAPP_CODE' and DELETOR=''"));
		$c_AREA=$r_TB_AREA['KODE_AREA'];
		$cFILTER_AREA=" and KODE_AREA='". $r_TB_AREA['KODE_AREA'] . "'";
		if (isset($_GET['_a'])) {
			if($_GET['_a']=='') {
				$cFILTER_AREA='';
			} else {
				$cFILTER_AREA=" and KODE_AREA='". $_GET['_a'] . "'";
			}
			$c_AREA=$_GET['_a'];
		}
	} else {
		$c_AREA=$c_RAYON;
		$cFILTER_AREA=" and KODE_AREA='". $c_AREA . "'";
	}

//	$r_TB_CATTER	= SYS_FETCH(SYS_QUERY("select * from bm_tb_catter1 where APP_CODE='$cAPP_CODE' and DELETOR=''"));

	$cHELP_BOX	= S_MSG('RQ8A','Help Laporan Rekap Baca Meter per Petugas');
	$cHELP_1	= S_MSG('RQ8B','Ini adalah modul untuk menampilkan Laporan Rekap Baca Meter per Petugas untuk periode tertentu');
	$cHELP_2	= S_MSG('RQ8C','Laporan ini memuat rekap jumlah hasil pembacaan meter untuk setiap Petugas dibanding dengan target masing-masing.');
	$cHELP_3	= S_MSG('RQ8D','Hasil pembacaan per Petugas ini adalah akumulasi pembacaan semua petugas');
							...
							<section class="box ">
								...
								<?php 
									if ($c_RAYON=='') {
										...
										echo '<select name="PILIH_AREA" class="col-sm-3 form-label-900" onchange="FILTER_DATA(' . "'". $dPERIOD1. "'". ', '. "'". $dPERIOD2. "'". ', this.value)">';
								...
								<input type="text" class="col-sm-2 form-label-900 datepicker" data-format="yyyy-mm-dd" value="<?php echo $dPERIOD1?>" onchange="FILTER_DATA(this.value, '<?php echo $dPERIOD2?>', '<?php echo $c_AREA?>')">

								<label class="col-sm-1 form-label-700" style="text-align: right"><?php echo S_MSG('RS14','s/d')?></label>
								<input type="text" class="col-sm-2 form-label-900 datepicker" data-format="yyyy-mm-dd" value="<?php echo $dPERIOD2?>" onchange="FILTER_DATA('<?php echo $dPERIOD1?>', this.value, '<?php echo $c_AREA?>')"><br>
								<div class="clearfix"></div>
...
<!--                                            <table cellspacing="0" id="myTable" class="table table-small-font table-bordered table-striped">	-->
												...
												<tbody>
													<?php
														...
														$t_BLM_TERBACA = 0;
														$t_CAPAIAN = 0;
														$q_TB_PTGS=SYS_QUERY("select KODE_CATTER, NAMA_CATTER, TARGET from bm_tb_catter1 where APP_CODE='$cAPP_CODE' and DELETOR='' $cFILTER_AREA");
														while($a_TB_PTGS =SYS_FETCH($q_TB_PTGS)){

															$n_TARGET = $a_TB_PTGS['TARGET'];
															$q_PELANGGAN = SYS_QUERY("select * from bm_tb_plg where PETUGAS ='$a_TB_PTGS[KODE_CATTER]' and APP_CODE='$cAPP_CODE' and DELETOR=''");
															$n_PELANGGAN = SYS_ROWS($q_PELANGGAN);
															$n_CAPAIAN = 0;
															$t_TARGET += $n_PELANGGAN;
															echo '<tr>';
															echo "<td><span>".$a_TB_PTGS['NAMA_CATTER']."  </span></td>";
															$nBLN = 0;	$n_JML_BLN=0;
															while($nBLN < intval(substr($dPERIOD2,5,2))) {
																$nBLN ++;
																$cSQLCOMMAND= "SELECT TGL_BACA, IDPEL, PETUGAS, KODE_RBM
																	from bm_dt_baca
																	where PETUGAS='$a_TB_PTGS[KODE_CATTER]' and APP_CODE='$cAPP_CODE' and DELETOR='' and
																		left(TGL_BACA,10)>='".$dPERIOD1."' and left(TGL_BACA,10)<='".$dPERIOD2."'";
...
case 'EXCEL' :
	header('Location:bm_rekap_ct.php');
//	header('Location:bm_xls_bc_mtr.php?_t1='.$dPERIOD1.'&_t2='.$dPERIOD2);
	mysql_close($DB2);
}
?>
<script>
function FILTER_DATA(p_TGL1, p_TGL2, p_AREA) {
	window.location.assign("bm_rekap_ct.php?_t1="+p_TGL1 + "&_t2="+p_TGL2 + "&_a="+p_AREA);
}
