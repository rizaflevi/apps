...
//	tb_branch.php //
	...
	$nGROUP_CUSTOMER = 0;
	$cQUERY=SYS_QUERY("select * from grouplgn where APP_CODE='$cFILTER_CODE' and DELETOR=''") or die ('Error in query.' .mysql_error());
	$nGROUP_CUSTOMER = SYS_ROWS($cQUERY);
	$cREC_SYS=SYS_FETCH(SYS_QUERY("select * from rainbow where KEY_FIELD='JNS_PRSHN' and APP_CODE='$cFILTER_CODE'"));
	
	$qQUERY=SYS_QUERY("select * from branch where APP_CODE='$cFILTER_CODE' and DELETOR=''");
	...
	$cACCT_CASH		= S_MSG('TB63','Account Kas Cabang');
	$cACCT_SALES	= S_MSG('TB64','Account Penjualan');
	$cACCT_COGS		= S_MSG('TB65','Account Harga Pokok');
	$cACCT_INV		= S_MSG('F120','Account Persediaan');
	$cACCT_RETR		= S_MSG('F121','Account Retur');
	$cACCT_UTANG	= S_MSG('F103','Account Utang');
	$cACCT_BIAYA	= S_MSG('TB69','Account Biaya Cbg.');
	$cACCT_DISKON	= S_MSG('GL12','(B) Diskon Penjualan');
	$cACCT_SETOR	= S_MSG('TB7B','Account setoran bank');
	$cSHUT_DOWN		= S_MSG('TB90','Langsung Shutdown');
	$cCOPY_BILL		= S_MSG('TB85','Cetak Nota');
	$cFORM_ENTRY	= S_MSG('SPB1','Form Sales Order');
	$cFORM_1		= 'P O S';
	$cFORM_2		= S_MSG('SPB2','Dengan List');
	$cFORM_3		= S_MSG('SPB3','Tanpa List');
	
	$cDATA_ACCOUNT	= S_MSG('TB61','Data Account');
	$cJNS_BYR		= S_MSG('TB41','Account Pembayaran');
	$cOTHERS		= S_MSG('TB71','Info Lainnya');
	...
	$ada_KOPERASI=0;
	if(substr($cREC_SYS['KEY_CONTEN'],23,1)!=' ') {
		$ada_KOPERASI=1;
	}
	...
	$ada_PPMK=0;
	if(substr($cREC_SYS['KEY_CONTEN'],21,1)!='') {
		$ada_PPMK=1;
	}
...
?>
	...
	<html class=" ">
		...
		<body class=" ">
								...
								<header class="panel_header">
									...
									<div class="pull-right">
										<ol class="breadcrumb">
											...
											<li>
												<a href="#help_tbl_pelanggan" data-toggle="modal" > <i class="fa fa-question"></i>Help</a>
											</li>
										</ol>
									</div>
								</header>
			...
			<div class="modal" id="help_tbl_pelanggan" tabindex="-1" role="dialog" aria-labelledby="ultraModal-Label" aria-hidden="true">
	...
	<!-- TAB - START of create -->
										<div class="col-sm-9">
											...
											<ul class="nav nav-tabs primary">
												 <li class="active">
													<a href="#ADD_ACCOUNT_BRANCH" data-toggle="tab">
														<i class="fa fa-user"></i> <?php echo $cDATA_ACCOUNT?>
													</a>
												 </li>
												 <li>
													<a href="#ADD_ACCOUNT_BAYAR" data-toggle="tab">
														<i class="fa fa-home"></i> <?php echo $cJNS_BYR?> 
													</a>
												 </li>
												 <li>
													<a href="#foto-0" data-toggle="tab">
														<i class="fa fa-cog"></i> <?php echo $cOTHERS?> 
													</a>
												 </li>
											</ul>

											<div class="tab-content primary">
												<div class="tab-pane fade in active" id="ADD_ACCOUNT_BRANCH">
													<label class="col-sm-4 form-label-700" for="field-4"><?php echo $cACCT_CASH?></label>
													<select name="ADD_GROUP" class="col-sm-7 form-label-900">
														<?php 
													...
													<label class="col-sm-4 form-label-700" for="field-4"><?php echo $cACCT_SALES?></label>
													<select name="ADD_TIPE" class="col-sm-7 form-label-900">
														<?php 
															echo "<option value=' '  > None </option>";
															$qQUERY=SYS_QUERY("select * from account where GENERAL='D' and APP_CODE='$cFILTER_CODE' and DELETOR=''");
															while($aREC_ACCOUNT=SYS_FETCH($qQUERY)){
													...
													<select name='ADD_COST_ACCT' class="col-sm-7 form-label-900">
														<?php 
															echo "<option value=' '  > None </option>";
															$qQUERY=SYS_QUERY("select * from account where GENERAL='D' and APP_CODE='$cFILTER_CODE' and DELETOR=''");
															while($aREC_ACCOUNT=SYS_FETCH($qQUERY)){
													...
													<select name="ADD_INV_ACCT" class="col-sm-7 form-label-900">
														<?php 
															echo "<option value=' '  > None </option>";
															$qQUERY=SYS_QUERY("select * from account where GENERAL='D' and APP_CODE='$cFILTER_CODE' and DELETOR=''");
															while($aREC_ACCOUNT=SYS_FETCH($qQUERY)){
													...
													<select name="ADD_RT_ACCOUNT" class="col-sm-7 form-label-900">
														<?php 
															echo "<option value=' '  > None </option>";
															$qQUERY=SYS_QUERY("select * from account where GENERAL='D' and APP_CODE='$cFILTER_CODE' and DELETOR=''");
															while($aREC_ACCOUNT=SYS_FETCH($qQUERY)){
													...
													<select name="ADD_AP_ACCOUNT" class="col-sm-7 form-label-900">
														<?php 
															echo "<option value=' '  > None </option>";
															$qQUERY=SYS_QUERY("select * from account where GENERAL='D' and APP_CODE='$cFILTER_CODE' and DELETOR=''");
															while($aREC_ACCOUNT=SYS_FETCH($qQUERY)){
													...
													<select name="ADD_DFLT_ACCT" class="col-sm-7 form-label-900">
														<?php 
															echo "<option value=' '  > None </option>";
															$qQUERY=SYS_QUERY("select * from account where GENERAL='D' and APP_CODE='$cFILTER_CODE' and DELETOR=''");
															while($aREC_ACCOUNT=SYS_FETCH($qQUERY)){
													...
													<select name="ADD_DISKP_ACCT" class="col-sm-7 form-label-900">
														<?php 
															echo "<option value=' '  > None </option>";
															$qQUERY=SYS_QUERY("select * from account where GENERAL='D' and APP_CODE='$cFILTER_CODE' and DELETOR=''");
															while($aREC_ACCOUNT=SYS_FETCH($qQUERY)){
													...
													<select name="ADD_SETOR_ACCT" class="col-sm-7 form-label-900">
														<?php 
															echo "<option value=' '  > None </option>";
															$qQUERY=SYS_QUERY("select * from account where GENERAL='D' and APP_CODE='$cFILTER_CODE' and DELETOR=''");
															while($aREC_ACCOUNT=SYS_FETCH($qQUERY)){
												...
												<div class="tab-pane fade" id="ADD_ACCOUNT_BAYAR">
													<label class="col-sm-4 form-label-700" for="field-1"><?php echo $cACCT_INV?></label>
													<select name="ADD_ACCOUNT" class="col-sm-4 form-label-900">
														<?php
															...
															$qQUERY=SYS_QUERY("select * from account where GENERAL='D' and APP_CODE='$cFILTER_CODE' and DELETOR=''");
															while($aREC_ACCOUNT=SYS_FETCH($qQUERY)){
																echo "<option value='$aREC_ACCOUNT[INV_ACCT]'  >$aREC_ACCOUNT[ACCT_NAME]</option>"; }
														?>
													</select>
													<div class="clearfix"></div>
												</div>
...
case md5('UPD_BRANCH'):
		$cQUERY ="select * from branch";
		$cQUERY.=" where APP_CODE='$cFILTER_CODE' and md5(BRCH_CODE)='$_GET[_c]' and DELETOR=''";
											...
											<div class="col-sm-9">
												...
												<ul class="nav nav-tabs primary">
													...
													<?php 
														if($ada_KOPERASI==0) {
															echo '<li>
																<a href="#TAB_UPD_OTHERS" data-toggle="tab">
																	<i class="fa fa-cog"></i>'. $cOTHERS;
																echo '</a>
															 </li>';
														}
													 ?>
												...
												<div class="tab-content primary">
													...
													<div class="tab-pane fade" id="PAY_ACCOUNT">
														<table id="example" class="<?php echo S_PARA('_DISP_TABLE_CLASS','display table table-hover table-condensed')?>">
															...
															<tbody>
																<?php
																	$qQUERY=SYS_QUERY("select * from pay_type where APP_CODE='$cFILTER_CODE' and DELETOR=''");
																	while($aREC_PAY_TYPE=SYS_FETCH($qQUERY)) {
																		echo '<tr>';
																			...
																			$q_PAY_BRCH=SYS_QUERY("select * from pay_brch where BRCH_CODE=$a_BRANCH[BRCH_CODE] and PAY_CODE='".$aREC_PAY_TYPE['PAY_CODE']. "' and APP_CODE='$cFILTER_CODE' and DELETOR=''");
													...
													<?php 
														if($ada_KOPERASI==0) {
															echo '<div class="tab-pane fade" id="TAB_UPD_OTHERS">';

																$cCHECK1 = '';
																$cCHECK2 = 'checked';
																if($a_BRANCH['OFF_DOWN'] == 1) {
																	$cCHECK1 = 'checked';
																	$cCHECK2 = '';
																}
																echo '<label class="col-sm-4 form-label-700" for="field-1">' . $cSHUT_DOWN.'</label>';
																echo '<input type="radio" name="UPD_OFF_DOWN" value=1 '.$cCHECK1. '/>' . 'Yes';
																echo '<input style="margin-left: 15px;" type="radio" name="UPD_OFF_DOWN" value=0 '.$cCHECK2. '/>' . 'No';
																echo '<div class="clearfix"></div>';

																$cCHECK1 = '';
																$cCHECK2 = '';
																$cCHECK3 = '';
																if($a_BRANCH['COPY_BILL'] == 1) {
																	$cCHECK1 = 'checked';
																}
																if($a_BRANCH['COPY_BILL'] == 2) {
																	$cCHECK2 = 'checked';
																}
																if($a_BRANCH['COPY_BILL'] == 3) {
																	$cCHECK3 = 'checked';
																}
																echo '<label class="col-sm-4 form-label-700" for="field-1">' . $cCOPY_BILL.'</label>';
																echo '<input type="radio" name="UPD_COPY_BILL" value=1 '.$cCHECK1. '/>' . '1 Copy';
																echo '<input style="margin-left: 50px;" type="radio" name="UPD_COPY_BILL" value=2 '.$cCHECK2. '/>' . '2 Copy';
																echo '<input style="margin-left: 50px;" type="radio" name="UPD_COPY_BILL" value=3 '.$cCHECK3. '/>' . '3 Copy';
																echo '<div class="clearfix"></div>';

																$cCHECK1 = '';
																$cCHECK2 = '';
																$cCHECK3 = '';
																if($a_BRANCH['FORM_ENTRY'] == 1) {
																	$cCHECK1 = 'checked';
																}
																if($a_BRANCH['FORM_ENTRY'] == 2) {
																	$cCHECK2 = 'checked';
																}
																if($a_BRANCH['FORM_ENTRY'] == 3) {
																	$cCHECK3 = 'checked';
																}
																echo '<label class="col-sm-4 form-label-700" for="field-1">' . $cFORM_ENTRY.'</label>';
																echo '<input type="radio" name="UPD_FORM_ENTRY" value=1 '.$cCHECK1. '/>' . $cFORM_1;
																echo '<input style="margin-left: 50px;" type="radio" name="UPD_FORM_ENTRY" value=2 '.$cCHECK2. '/>' . $cFORM_2;
																echo '<input style="margin-left: 50px;" type="radio" name="UPD_FORM_ENTRY" value=3 '.$cCHECK3. '/>' . $cFORM_3;
																echo '<div class="clearfix"></div>';
															echo '</div>';
														}
													?>
...
case 'tambah':
	...
	if($cBRCH_CODE=='') {
		$cMSG_BLANK=S_MSG('F040','Kode Pelanggan tidak boleh kosong');
		echo "<script> alert('$cMSG_BLANK');	window.history.back();	</script>";
		return;
	}

	$cQUERY="select * from langgan where APP_CODE='$cFILTER_CODE' and BRCH_CODE='$cBRCH_CODE' and DELETOR=''";
	...
	if(SYS_ROWS($cCEK_KODE)==0){
		...
		$cQUERY ="insert into langgan set APP_CODE='$cFILTER_CODE', BRCH_CODE='$cBRCH_CODE', BRCH_NAME='$cBRCH_NAME', ";
...
case 'rubah':
	...
	$cQUERY ="update branch set BRCH_NAME='$cBRCH_NAME',
		...
		if($ada_KOPERASI==0 and isset($_POST['$UPD_FORM_ENTRY'])) { $cQUERY.=" FORM_ENTRY='$_POST[UPD_FORM_ENTRY]', "; }
	$cQUERY.=" UP_DATE='$_SESSION[gUSERCODE]', UPD_DATE='$NOW' ";
	$cQUERY.="where APP_CODE='$cFILTER_CODE' and BRCH_CODE='$KODE_BRCH'";
	SYS_QUERY($cQUERY);

	$qQUERY=SYS_QUERY("select * from pay_type where APP_CODE='$cFILTER_CODE' and DELETOR=''");
	while($aREC_PAY_TYPE=SYS_FETCH($qQUERY)) {
		$q_PAY_BRCH=SYS_QUERY("select * from pay_brch where BRCH_CODE='$KODE_BRCH' and PAY_CODE='".$aREC_PAY_TYPE['PAY_CODE']. "' and APP_CODE='$cFILTER_CODE' and DELETOR=''");
		$aREC_PAY_BRCH=SYS_FETCH($q_PAY_BRCH);
		$lCHECK = 0;
		...
		if ($nCHECK == '1') {
			if(SYS_ROWS($q_PAY_BRCH)>0){
				$q_UPD_PAY_BRCH = SYS_QUERY("update pay_brch set DI_ACCEPT=1 where BRCH_CODE='$KODE_BRCH' and PAY_CODE='" .$aREC_PAY_TYPE['PAY_CODE']. "' and APP_CODE='$cFILTER_CODE' and DELETOR=''");
			} else {
				$q_UPD_PAY_BRCH = SYS_QUERY("insert into pay_brch set DI_ACCEPT=1, BRCH_CODE='$KODE_BRCH', PAY_CODE='$aREC_PAY_TYPE[PAY_CODE]', APP_CODE='$cFILTER_CODE'");
			}
		} else {
			if(SYS_ROWS($q_PAY_BRCH)>0){
				$q_UPD_PAY_BRCH = SYS_QUERY("update pay_brch set DI_ACCEPT=0 where BRCH_CODE='$KODE_BRCH' and PAY_CODE='".$aREC_PAY_TYPE['PAY_CODE']. "' and APP_CODE='$cFILTER_CODE' and DELETOR=''");
			}
		}
...
case md5('del_branch'):
	...
	$cQUERY ="update langgan set DELETOR='$_SESSION[gUSERCODE]', DEL_DATE='$NOW' 
		where APP_CODE='$cFILTER_CODE' and md5(BRCH_CODE)='$KODE_CRUD'";
	SYS_QUERY($cQUERY);	// ;
	header('location:tb_branch.php');
}
?>
