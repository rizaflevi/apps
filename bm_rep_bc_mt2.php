...
//	include "preloader.php";
	...
	if ($c_RAYON=='') {
		...
		if (isset($_POST['_a'])) {
			if($_POST['_a']=='') {
//	var_dump($_POST['_a']); exit;
				$cFILTER_AREA='';
			} else {
				$cFILTER_AREA=" and UNITUP='". $_POST['_a'] . "'";
			}
			$c_AREA=$_POST['_a'];
		}
	} else {
...
//	$r_TB_CATTER	= SYS_FETCH(SYS_QUERY("select * from bm_tb_catter1 where APP_CODE='$cFILTER_CODE' and DELETOR=''"));
	...
	$cSQLCOMMAND= "SELECT A.TGL_BACA, A.IDPEL, A.SISA_TOKEN, A.LATTI, A.LONGI, A.PETUGAS, A.KODE_RBM, A.KONDISI_RMH, A.SEGEL_TLG, A.SEGEL_TER, A.LAMPU_INDI, A.NOTE, B.TRF_TPSG, C.IDPEL, C.UNITUP, C.NOMOR_KWH, C.NAMA_PEL, C.KODE_TARIF, C.DAYA, C.ALAMAT
		from bm_dt_baca A
		inner join (select IDPEL, TRF_TPSG, PETUGAS, KODE_RUTE, LAST_VISIT FROM bm_tb_plg where APP_CODE='$cFILTER_CODE' and DELETOR='' $cFILTER_PTGS) B ON B.IDPEL=A.IDPEL 
		...
		where A.APP_CODE='$cFILTER_CODE' and A.DELETOR='' $nFILTER_RBM and
			left(A.TGL_BACA,10)>='".$dPERIOD1."' and left(A.TGL_BACA,10)<='".$dPERIOD2."' order by A.TGL_BACA desc";
					...
					<section class="wrapper main-wrapper" style=''>
						...
						<div class="col-xs-12">
							<section class="box ">
								...
								<form action ="?_f=r" method="post">
								<?php 
									if ($c_RAYON=='') {
										echo '<label class="col-sm-1 form-label-700" for="field-4">'. $cAREA.'</label>';
										echo '<select name="_a" class="col-sm-3 form-label-900">';
								...
								<input type="text" class="col-sm-2 form-label-900 datepicker" name='_t1' data-format="yyyy-mm-dd" value="<?php echo $dPERIOD1?>">

								<label class="col-sm-1 form-label-700" style="text-align: right"><?php echo S_MSG('RS14','s/d')?></label>
								<input type="text" class="col-sm-2 form-label-900 datepicker" name='_t2' data-format="yyyy-mm-dd" value="<?php echo $dPERIOD2?>"><br>
								<div class="clearfix"></div>
								...
								<?php 
									...
									echo '<select name="_a" class="col-sm-2 form-label-900" value="' . $c_PTGS.'">';
								...
								<div class="text-left">
									...
									<button type="submit" class="btn btn-secondary btn-icon bottom15 right15">
										<i class="fa fa-refresh"></i> &nbsp; <span>Re-load</span>
									</button>
...
case 'EXCEL' :
	header('Location:bm_xls_bc_mtr.php?_t1='.$dPERIOD1.'&_t2='.$dPERIOD2.'&_c='.$c_PTGS.'&_r='.$nFILTER_RBM);
	mysql_close($DB2);
}
?>
<script>
