...
$cHEADER 		= S_MSG('DA22','Daftar Penerimaan');
$cKD_BYR 	= S_MSG('NR02','No. Penerimaan');
...
$qFONT= OpenTable('TbFont', "KEY_ID='RECEIPT_COM' and APP_CODE='$cAPP_CODE' and REC_ID not in ( select DEL_ID from logs_delete )");
...
$nROW=$mROW=6;
$qQUERY=OpenTable('TrReceiptHdr', "A.APP_CODE='$cAPP_CODE' and A.REC_ID not in ( select DEL_ID from logs_delete) and A.TGL_BAYAR>='$dPERIOD1' and A.TGL_BAYAR<='$dPERIOD2'", '', 'A.TGL_BAYAR');
while($aREC_PAY=SYS_FETCH($qQUERY)) {
    $nROW++;
    CELL_BY_COL_ROW($sheet, 1, $nROW, $aREC_PAY['NO_TRM']);
    $dPAY_DATE = strtotime( $aREC_PAY['TGL_BAYAR'] );
    $cPAY_DATE = date( 'd/m/Y', $dPAY_DATE );
    CELL_BY_COL_ROW($sheet, 2, $nROW, $cPAY_DATE);
    CELL_BY_COL_ROW($sheet, 3, $nROW, DECODE($aREC_PAY['DESCRP']));
    $nAMOUNT =0;
    $dQUERY=OpenTable('TrReceiptDtl', "A.NO_TRM='$aREC_PAY[NO_TRM]' and A.APP_CODE='$cAPP_CODE' and A.REC_ID not in ( select DEL_ID from logs_delete)");
