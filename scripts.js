        ...
        $('.page-chatapi .user-row').on('click', function() {
            ...
            if ($(this).hasClass("active")) {
                ...
                if ($(".chatapi-windows #user-window" + id).length) {
                    ...
                    msg += chatformat_msg('Yes! Rainbow System ;)', 'sent', 'You');
...
//                        $this.find('span').html(start.format(drp.format) + drp.separator + end.format(drp.format));
        ...
        if ($.isFunction($.fn.select2)) {
            ...
            $("#s_customer").select2({
                ...
                loadPersonel(e.val, $('#s_customer option:selected'))
                loadArea(e.val, $('#s_customer option:selected'))
            });

            function loadPersonel(customer, element) {
                $.get(location.pathname, {_a: 'personel', _cc: customer, _cd: element.attr('data-id')}, function(res) {
                    $("#s2personel").select2({
                        placeholder: 'Select your personel...',
                        allowClear: true,
                        data: JSON.parse(res)
                    }).on('change', function (e) {
                        console.log(e.val);
            ...
            function loadArea(customer, element) {
                $.get(location.pathname, {_a: 'area', _cc: customer, _cd: element.attr('data-id')}, function(res) {
                    $("#s2area").select2({
                        placeholder: 'Select your area...',
                        allowClear: true,
                        data: JSON.parse(res)
                    }).on('change', function (e) {
                        loadOptionList(e.val);
                    });
                });
            }

            function loadOptionList(area) {
                $.get(location.pathname, {_a: 'options', area: area}, function(res) {
                    $('div#option-list').html(res)
                })
            }
