<?php
//	bm_rep_bc_mt_reload.php
//	Laporan baca meter, Brb =979, Brb A =185, Brb A, Arifin =49
//	include "preloader.php";
	...
	if (isset($_POST['TGL_AWAL'])) {
		$dPERIOD1=$_POST['TGL_AWAL'];
	}

	if (isset($_POST['TGL_AKHIR'])) {
		$dPERIOD2=$_POST['TGL_AKHIR'];
	}
	...
	if ($c_RAYON=='') {
		...
		if (isset($_POST['RAYON'])) {
			if($_POST['RAYON']=='') {
				...
				$cFILTER_AREA=" and UNITUP='". $_POST['RAYON'] . "'";
			}
			$c_AREA=$_POST['RAYON'];
		}
	} else {
...
//	$r_TB_CATTER	= SYS_FETCH(SYS_QUERY("select * from bm_tb_catter1 where APP_CODE='$cFILTER_CODE' and DELETOR=''"));
	...
	if (isset($_POST['RBM'])) {
		if($_POST['RBM']=='') {
			...
			$nFILTER_RBM=" and KODE_RUTE='". $_POST['RBM'] ."'";
		}
		$c_RBM=$_POST['RBM'];
	}
	...
	$c_PTGS='';
	$cFILTER_PTGS='';
	if (isset($_POST['PILIH_PTGS'])) {
		if($_POST['PILIH_PTGS']=='') {
			$cFILTER_PTGS='';
		} else {
			$cFILTER_PTGS=" and A.PETUGAS='".$_POST['PILIH_PTGS']."'";
		}
		$c_PTGS=$_POST['PILIH_PTGS'];
	}
							...
							<section class="box ">
								...
								<?php 
									if ($c_RAYON=='') {
										...
										echo '<select name="RAYON" class="col-sm-3 form-label-900">';
								...
								<input type="text" name="TGL_AWAL" class="col-sm-2 form-label-900 datepicker" data-format="yyyy-mm-dd" value="<?php echo $dPERIOD1?>">

								<label class="col-sm-1 form-label-700" style="text-align: right"><?php echo S_MSG('RS14','s/d')?></label>
								<input type="text" name="TGL_AKHIR" class="col-sm-2 form-label-900 datepicker" data-format="yyyy-mm-dd" value="<?php echo $dPERIOD2?>"><br>
								<div class="clearfix"></div>

								<label class="col-sm-1 form-label-700"><?php echo $cRUTE_BACA?></label>
								<select name="RBM" class="col-sm-1 form-label-900">
								<?php
								...
								<select name="PILIH_PTGS" class="col-sm-2 form-label-900" value=<?php echo $c_PTGS?>>'
								<?php 
