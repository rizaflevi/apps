...
//	bm_pelanggan.php //
	...
	$cQUERY="select bm_tb_pel.IDPEL, bm_tb_pel.NAMA_PEL, bm_tb_pel.ALAMAT, bm_tb_pel.KODE_TARIF, bm_tb_pel.DAYA, bm_tb_pel.NOMOR_KWH, bm_tb_pel.MERK_KWH, bm_tb_pel.UNITUP,
		bm_tb_plg.IDPEL, bm_tb_plg.HARI_KUNJ, bm_tb_plg.PETUGAS, bm_tb_plg.KODE_RUTE, bm_tb_plg.NO_URUT
		from bm_tb_pel
	...
	$cQUERY.= " and bm_tb_plg.HARI_KUNJ=".$nFILTER_HARI;
	$cQUERY.= " and bm_tb_plg.PETUGAS='".$cFILTER_PTGS."'";
	if ($nFILTER_HARI==0) {
		$cQUERY.= " limit 10";
	}
//	var_dump($cQUERY); exit();
	
	$qQUERY=SYS_QUERY($cQUERY);		// ." limit 1000"
								...
								<div class="row">
									<form action ="?_a=tambah" method="post">
										<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
											...
											<input type="text" class="col-sm-2 form-label-900" name='ADD_IDPEL' title="<?php echo $cTTIP_KODE?>">
											<div class="clearfix"></div>
											...
											<input type="text" class="col-sm-5 form-label-900" name='ADD_ALAMAT' title="<?php echo $cTTIP_ALMT?>">
											<div class="clearfix"></div>

											<label class="col-sm-4 form-label-700" for="field-4"><?php echo $cAREA?></label>
											<select name="ADD_UNITUP" class="col-sm-4 form-label-900">
											<?php 
												$q_TB_AREA=SYS_QUERY("select * from tb_area where APP_CODE='$cFILTER_CODE' and DELETOR=''");
//												echo "<option value=' '  > </option>";
											...
											<input type="text" class="col-sm-8 form-label-900" name='ADD_NOMOR_KWH' id="field-2">
	...
	<html class=" ">
			...
			<div class="page-container row-fluid">
						...
						<div class='col-lg-9 col-md-9 col-sm-9 col-xs-9'>
							<div class="page-title">
								...
								<div class="pull-right hidden-xs">									 
									<ol class="breadcrumb">
										<li>
											<?php echo '<a href="?_a='.md5('del_area').'&id='. md5($aREC_BM_TB_PEL['IDPEL']). '" onClick="return confirm('. "'". $cHAPUS. "'". ')"><i class="glyphicon glyphicon-minus-sign"></i>Delete</a>' ?>	 
										</li>
									</ol>
								</div>
							</div>
									...
									<div class="row">
										<form action ="?_a=rubah&id=<?php echo $aREC_BM_TB_PEL['IDPEL']?>" method="post">
											<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
												...
												<label class="col-sm-4 form-label-700" for="field-1"><?php echo $cALAMAT?></label>
												<input type="text" class="col-sm-5 form-label-900" name='EDIT_ALAMAT' value="<?php echo decode_string($aREC_BM_TB_PEL['ALAMAT'])?>" title="<?php echo $cTTIP_ALMT?>">
												<div class="clearfix"></div>
				...
				//									echo "<option value=''  > All</option>";
												...
												<label class="col-sm-4 form-label-700"><?php echo $cHARI_BACA?></label>
												<input type="text" class="col-sm-1 form-label-900" name='UPD_HARI_KUNJ' value="<?php echo $aREC_BM_TB_PLG['HARI_KUNJ']?>">
												<div class="clearfix"></div>
...
case 'tambah':
	...
	if(SYS_ROWS($cCEK_KODE)>0){
		...
		SYS_QUERY($cQUERY);
		APP_LOG_ADD($cHEADER, 'add : '.$cIDPEL);
...
case 'rubah':
	...
	$cDESC_CATTER	= encode_string($_POST['UPD_NOMOR_KWH']);	
	$cALAMAT		= encode_string($_POST['EDIT_ALAMAT']);	
	$cQUERY ="update bm_tb_pel set NAMA_PEL='$cNAMA_PEL', ALAMAT='$cALAMAT', NOMOR_KWH='$cDESC_CATTER', UNITUP='$_POST[UPD_UNITUP]', KODE_TARIF='$_POST[UPD_KODE_TARIF]', 
		UP_DATE='$_SESSION[gUSERCODE]', UPD_DATE='$NOW'
		where APP_CODE='$cFILTER_CODE' and IDPEL='$KODE_CRUD'";
	SYS_QUERY($cQUERY);
	$cQUERY ="update bm_tb_plg set PETUGAS='$_POST[UPD_PETUGAS]', HARI_KUNJ='$_POST[UPD_HARI_KUNJ]', KODE_RUTE='$_POST[UPD_KODE_RUTE]', NO_URUT='$_POST[UPD_NO_URUT]', 
		UP_DATE='$_SESSION[gUSERCODE]', UPD_DATE='$NOW'
		where APP_CODE='$cFILTER_CODE' and IDPEL='$KODE_CRUD'";
	SYS_QUERY($cQUERY);
	APP_LOG_ADD($cHEADER, 'edit : '.$KODE_CRUD);
	header('location:bm_pelanggan.php?_r='.$cFILTER_AREA.'&_h='.$nFILTER_HARI.'&_p='.$cFILTER_PTGS);
	break;
